...
def test_module1_with_mocked_openai(mock_openai_class):
    ...
    mock_client.chat = SimpleNamespace(completions=SimpleNamespace(create=mock_create))
        ...
        if messages and messages[0]["content"]:
            ...
            if "SearchAgent" in content or "Find information" in content:
                return create_mock_response(
                    "Mocked search summary about success criteria."
                )
                ...
                "CriteriaEvaluator" in content or "select the most relevant" in content
            ):
                return create_mock_response(
                    json.dumps(
                        [
                            {
                                "criteria": "Mock Selected Criterion",
                                "reasoning": "Selected as best",
                                "rating": 9,
                            }
                        ]
                    )
    ...
    async def test_agent_calls():
        ...
        result = await search_agent.run(
            mock_client, "Find information about test prompt"
        )
        assert "search summary" in result.lower()

        # Test CriteriaGenerator
        result = await generate_criteria_agent.run(mock_client, "test prompt")
        assert len(result) == 2
        assert result[0].criteria == "Mock Criterion 1"

        # Test CriteriaEvaluator
        result = await evaluate_criteria_agent.run(mock_client, "test prompt")
        assert len(result) == 1
